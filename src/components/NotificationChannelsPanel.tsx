...
/** Couleurs BLAFFA — #003399 / #002d72 / orange-500 */
const CHANNEL_ACCENT = 'border-[#002d72]/30 ring-1 ring-[#003399]/15';
const BTN_PRIMARY = 'bg-[#002d72] hover:bg-[#001d4a] text-white';
const BTN_OUTLINE = 'border border-[#002d72] text-[#002d72] dark:text-blue-300';
const ICON_BRAND = 'text-[#002d72] dark:text-blue-400';
const SELECT_CLASS =
  'w-32 shrink-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-[#002d72] focus:border-transparent dark:border-slate-500 dark:bg-slate-700 dark:text-white';
const INPUT_CLASS =
  'min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-[#002d72] focus:border-transparent dark:border-slate-500 dark:bg-slate-700 dark:text-white dark:placeholder:text-slate-400';
...
export function splitPhoneNumber(fullPhone: string, indicative: string) {
  ...
  // Sinon, détecter l'indicatif directement depuis le numéro (ex: 229... alors que le profil est +225)
...
export function getDefaultPhonesFromUser(user: Record<string, unknown> | null | undefined) {
  ...
  return {
    // Si un numéro WhatsApp est déjà enregistré, son indicatif détecté prime sur celui du profil
    ...
    <ol className="mt-3 space-y-2">
      {steps.map((step, index) => (
        <li key={step} className="flex gap-3 text-xs leading-5 text-slate-600 dark:text-slate-300">
          <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#003399]/10 text-[11px] font-bold text-[#002d72] dark:bg-[#003399]/25 dark:text-blue-300">
            {index + 1}
          </span>
          <span>{step}</span>
        </li>
      ...
      {alwaysOpen ? (
        <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs">
                {done ? (
                  <span className="text-[#002d72] dark:text-blue-400">Connecté</span>
                ) : (
          ...
          {done && <CheckCircle2 className="h-5 w-5 text-[#002d72] dark:text-blue-400" />}
        </div>
      ) : (
        <button
          type="button"
        ...
        >
          ...
          <div className="flex items-center gap-2">
            {done && !open ? (
              <CheckCircle2 className="h-5 w-5 text-[#002d72] dark:text-blue-400" />
            ) : (
...
export default function NotificationChannelsPanel({
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (user?.user_telegram_username) {
        setTelegramUsername(user.user_telegram_username);
      }

      if (useTelegram) {
        try {
          const linkResponse = await api.get('/auth/telegram-link');
          setTelegramLink(linkResponse.data?.link || '');
        } catch {
          setTelegramLink('');
        }
      }
  ...
  const checkAllDone = useCallback(
    (waPending: boolean, tgPending: boolean, smsPending: boolean) => {
      if (!waPending && !tgPending && !smsPending) {
        onAllDone?.();
        onStatusChange?.();
      }
    },
    [onAllDone, onStatusChange]
  );
  ...
  const collapseIfProfile = () => {
    // En mode profil, les 3 canaux restent ouverts ensemble : rien à replier.
  };
  ...
  const openTelegramBot = async (link?: string) => {
    ...
    setSuccess('Bot BLAFFA ouvert. Appuyez sur Démarrer / Start, puis revenez ici.');
  };
  ...
  const handleTelegramSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (message === 'USER_MUST_START_BOT' && botLink) {
        ...
        setError('Appuyez sur Start dans le bot BLAFFA, puis réessayez.');
      } else {
        setError('Démarrez le bot BLAFFA (Start), puis réessayez.');
      }
    } finally {
      setLoading(false);
  ...
  if (!showWhatsapp && !showTelegram && !showSms) {
    return (
      <div className="py-8 text-center">
        <CheckCircle2 className="mx-auto mb-3 h-10 w-10 text-[#002d72] dark:text-blue-400" />
    ...
    <div>
      {showHeader && (
        <div className="mb-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#003399]/10 dark:bg-[#003399]/25">
      ...
      <div className={`space-y-3 ${isProfileMode && !profileExpanded ? 'hidden' : ''}`}>
        {showWhatsapp && (
          ...
          >
            {whatsappVerified && (
              <p className="mb-3 text-xs text-[#002d72] dark:text-blue-400">
          ...
          >
            ...
            <StepList
              steps={[
                ...
                'Le bot BLAFFA s\'ouvre automatiquement — appuyez sur Démarrer / Start.',
            ...
            <form onSubmit={handleTelegramSubmit} className="mt-4 space-y-3">
              ...
              {pollingTelegram && (
                <p className="text-center text-xs text-[#002d72] dark:text-blue-400">En attente de connexion au bot...</p>
              )}
            </form>
          </ChannelCard>
        )}
          ...
          >
            ...
            <StepList
              steps={[
                ...
                'Vos alertes BLAFFA arriveront ensuite par SMS.',
              ]}
            />
      ...
      {success && (
        <p className="mt-3 rounded-xl bg-[#003399]/8 px-4 py-3 text-sm text-[#002d72] dark:bg-[#003399]/20 dark:text-blue-300">
          {success}
        </p>
      )}
      ...
      {showLaterButton && (
        <button
          type="button"
          onClick={handleLater}
          className={`mt-4 w-full rounded-xl px-4 py-3 text-sm font-semibold ${BTN_OUTLINE} dark:border-blue-400`}
        >
          Plus tard
        </button>
      )}
