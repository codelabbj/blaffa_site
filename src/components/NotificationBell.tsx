...
// import { BellIcon } from '@heroicons/react/24/outline'; // Make sure this matches your project's import pattern
...
// /**
//  * Direct API call to mark a notification as read
//  * This function can be exported and used anywhere in the application
//  * @param notificationId - The ID of the notification to mark as read
//  * @returns Promise resolving to the API response or null if error
//  */
...
//       `https://api.lamedcash.com/lamedcash/notification/${notificationId}/mark-read/`, 
//       {
//         method: 'POST',
//         headers: { 
...
//   // Store previously marked read notifications
...
//   // Save marked-as-read cache to localStorage whenever it changes
...
//     try {
//       setLoading(true);
//       const res = await fetch(`https://api.lamedcash.com/lamedcash/notification?page=${pageNum}`, {
...
//           // Mark any notifications as read on the server that are marked locally but not on server
//           cleaned.forEach((notif: Notification) => {
//             if (markedAsReadCache.current.has(notif.id) && !notif.is_read) {
//               markAsReadSilent(notif.id);
//             }
//           });
//         } else {
...
//       // Send to server
//       await fetch(`https://api.lamedcash.com/lamedcash/notification/${id}/mark-read/`, {
//         method: 'POST',
//         headers: { Authorization: `Bearer ${accessToken}` },
//       });
//     } catch (err) {
...
//   // Silent version that doesn't update UI state (for bulk operations)
//   const markAsReadSilent = async (id: string) => {
//     const accessToken = localStorage.getItem('accessToken');
//     if (!accessToken) return;
...
//     // Make individual API calls in the background
//     const unreadNotifications = notifications.filter(n => !n.is_read);
    
//     for (const notification of unreadNotifications) {
//       try {
//         await fetch(`https://api.lamedcash.com/lamedcash/notification/${notification.id}/mark-read/`, {
//           method: 'POST',
//           headers: { Authorization: `Bearer ${accessToken}` },
//         });
//       } catch (err) {
...
//       const wsUrl = `wss://api.lamedcash.com/ws/socket?token=${encodeURIComponent(accessToken)}`;
...
//       {open && (
//         <div className="absolute right-0 mt-2 w-80 max-h-[28rem] overflow-y-auto bg-white dark:bg-gray-900 text-green-700 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-50">
//           <div className="p-4">
